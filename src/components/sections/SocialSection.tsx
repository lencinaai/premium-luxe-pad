        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {socialSection.links.map((link) => (
            <a
              ...
              className="flex items-center gap-3 bg-card border border-border rounded-lg px-4 py-3.5 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg group"
              style={{
                boxShadow: `inset 0 1px 2px 0 hsl(0 0% 100% / 0.04)`,
              }}
              onMouseEnter={(e) => {
                (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 20px ${link.brandColor}18`;
                (e.currentTarget as HTMLElement).style.borderColor = `${link.brandColor}33`;
              }}
              onMouseLeave={(e) => {
            ...
            >
              <span
                className="w-9 h-9 rounded-full flex items-center justify-center border border-border bg-secondary/60 flex-shrink-0"
              >
                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current" style={{ color: link.brandColor }}>
                  <path d={getIconPath(link.icon)} />
                </svg>
              </span>
              <span className="text-foreground text-sm font-medium truncate">{link.label}</span>
            </a>
          ))}
        </div>
      </div>
...
function getIconPath(icon: string) {
  return iconPaths[icon] || iconPaths.instagram;
}
